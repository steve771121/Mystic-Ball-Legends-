...
import {ARENAS,GOAL} from '../config/game.js';
export function arena(type){const group=new T.Group(),cfg=ARENAS[type];
 ...
 let shell=material(0x1b2a35,.7),trim=material(0x809b9b,.8,.22);box(11,.7,17,0,-.12,0,shell);box(10,.2,16,0,.32,0,new T.MeshPhysicalMaterial({color:cfg.surface,metalness:.4,roughness:.23,clearcoat:1}));
 for(let side of [-1,1]){box(.25,.5,16.4,side*5.13,.57,0,trim);box(.045,.045,16,side*4.99,.83,0,glow(cfg.color));for(let x of [-1,1])box((10-GOAL)/2,.5,.25,x*(GOAL/2+(10-GOAL)/4),.57,side*8.12,trim);
 ...
 const dots=new T.InstancedMesh(new T.CircleGeometry(.012,4),line,640);let index=0;let dummy=new T.Object3D();dummy.rotation.x=-Math.PI/2;for(let x=-4.5;x<5;x+=.5)for(let z=-7.5;z<8;z+=.5){dummy.position.set(x,.433,z);dummy.updateMatrix();dots.setMatrixAt(index++,dummy.matrix);}dots.count=index;group.add(dots);

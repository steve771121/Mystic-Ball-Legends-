import {ARENAS,W,H,GOAL,clamp,legalSkill,COOLDOWN} from '../config/game.js';
...
export class Physics {
 ...
 input(i,x,z){if(!Number.isFinite(x)||!Number.isFinite(z)||!this.players[i])return;let m=this.players[i];m.tx=clamp(x,-4.35,4.35);m.tz=clamp(z*m.side,.72,7.22)*m.side;}
 ...
 if(p.x<-5+p.r){p.x=-5+p.r;p.vx=Math.abs(p.vx);this.events.push({type:'wall',x:p.x,z:p.z});}if(p.x>5-p.r){p.x=5-p.r;p.vx=-Math.abs(p.vx);this.events.push({type:'wall',x:p.x,z:p.z});}
 ...
 if(Math.abs(p.x)>GOAL/2-p.r){if(p.z<-8+p.r){p.z=-8+p.r;p.vz=Math.abs(p.vz);}if(p.z>8-p.r){p.z=8-p.r;p.vz=-Math.abs(p.vz);}}

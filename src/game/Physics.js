import {portalState,updatePortals,enterPortal} from './Portals.js?v=0.5.0';
import {roundCorner} from './Bounds.js?v=0.5.0';
import {ARENAS,W,H,CORNER_RADIUS,GOAL,clamp,legalSkill,COOLDOWN} from '../config/game.js?v=0.5.0';
...
export class Physics {
 ...
 if(Math.abs(p.z)<=H/2-CORNER_RADIUS&&p.x<-5+p.r){p.x=-5+p.r;p.vx=Math.abs(p.vx);this.recordWall();}if(Math.abs(p.z)<=H/2-CORNER_RADIUS&&p.x>5-p.r){p.x=5-p.r;p.vx=-Math.abs(p.vx);this.recordWall();}
 ...
 if(Math.abs(p.x)>GOAL/2-p.r&&Math.abs(p.x)<=W/2-CORNER_RADIUS){if(p.z<-8+p.r){p.z=-8+p.r;p.vz=Math.abs(p.vz);this.recordWall();}if(p.z>8-p.r){p.z=8-p.r;p.vz=-Math.abs(p.vz);this.recordWall();}}
 ...
 recordWall(){this.impact={t:this.t};this.events.push({type:'wall',x:this.puck.x,z:this.puck.z});}

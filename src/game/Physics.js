import {portalState,updatePortals,enterPortal} from './Portals.js?v=0.6.0';
import {roundCorner} from './Bounds.js?v=0.6.0';
import {ARENAS,W,H,MALLET_RADIUS,CORNER_RADIUS,GOAL,clamp,legalSkill,COOLDOWN} from '../config/game.js?v=0.6.0';
...
export class Physics {
 ...
 resetRound(){this.impact=null;this.portals=portalState();this.players=[mallet(1),mallet(-1)];Object.assign(this.puck,{x:0,z:0,vx:0,vz:0,held:-1,hidden:false});this.skills=[null,null];this.phase='countdown';this.wait=3;}
 input(i,x,z){if(!Number.isFinite(x)||!Number.isFinite(z)||!this.players[i])return;let m=this.players[i];m.tx=clamp(x,-(4.35-(m.r-.62)),4.35-(m.r-.62));m.tz=clamp(z*m.side,.72,7.22)*m.side;const target={x:m.tx,z:m.tz,r:m.r};roundCorner(target);m.tx=target.x;m.tz=target.z;}
 ...
 const p=this.puck;const inTransit=updatePortals(this,dt);
 ...
 for(let i=0;i<2;i++){let s=this.skills[i];if(!s)continue;s.age+=dt;if(s.age>=5){if(p.held===i){p.held=-1;p.vz=-s.side*2;}this.skills[i]=null;continue;}if(!inTransit&&s.type==='duck'&&s.age>s.released){let dx=s.x-p.x,dz=s.z-p.z,d=Math.hypot(dx,dz);if(p.held===i){p.x=s.x;p.z=s.z;p.vx=p.vz=0;}else if(p.held<0&&d<2.0){p.vx+=dx*25*dt;p.vz+=dz*25*dt;p.vx*=Math.exp(-2.8*dt);p.vz*=Math.exp(-2.8*dt);if(d<.34){p.held=i;p.x=s.x;p.z=s.z;p.vx=p.vz=0;}}}}
 if(inTransit)return;
 ...
 if(this.config.arena==='space'&&enterPortal(this))return;
 ...
 if(Math.abs(p.z)<=H/2-CORNER_RADIUS&&p.x<-5+p.r){p.x=-5+p.r;p.vx=Math.abs(p.vx);this.recordWall(Math.abs(p.vx));}if(Math.abs(p.z)<=H/2-CORNER_RADIUS&&p.x>5-p.r){p.x=5-p.r;p.vx=-Math.abs(p.vx);this.recordWall(Math.abs(p.vx));}
 // Circular posts stop grazing shots; only the entire puck may cross the goal line.
 for(const z of [-8,8])for(const x of [-GOAL/2,GOAL/2])this.collide({x,z,r:.14,vx:0,vz:0,side:z>0?1:-1},1,'wall');
 if(Math.abs(p.x)>GOAL/2-p.r&&Math.abs(p.x)<=W/2-CORNER_RADIUS){if(p.z<-8+p.r){p.z=-8+p.r;p.vz=Math.abs(p.vz);this.recordWall(Math.abs(p.vz));}if(p.z>8-p.r){p.z=8-p.r;p.vz=-Math.abs(p.vz);this.recordWall(Math.abs(p.vz));}}
 if(roundCorner(p,true))this.recordWall();
 ...
 apply(s){for(const k of ['impact','portals','t','scores','players','puck','skills','cooldowns','phase','wait','winner','goalId','lastScorer'])this[k]=s[k];}
}
